...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@RequestBody RegisterRequestDTO registerRequest) {
        ...
        try {
            ...
            // Convert Set<Role> to List<String> for notification
            List<String> roleNamesList = registerRequest.getRoles().stream()
                    .map(Role::name)
                    .collect(Collectors.toList());

            String roleNamesStr = String.join(", ", roleNamesList);

            // Notify admin users (or any role, based on logic)
            notificationService.notifyRoles(
                    roleNamesList,
                    "New user registered with role(s): " + roleNamesStr
            );
